...
- [Concurrency, timeouts, and what to change when users complain](CONCURRENCY_TUNING.md) — the six ceilings a graph request passes through, the rule that orders their deadlines, and a troubleshooting guide keyed by what the user reports rather than by variable name.
