...
- Split graph_cache's Redis usage: response-cache payloads + LKG use a CACHE-role client resolved via `resolve_redis_config(RedisRole.CACHE)` (honors global `REDIS_CACHE_*` and the per-provider `cacheConnection` override, precedence per `build_cache_client`), FALLING BACK to the current shared/bus Redis when no cache is configured (no dev regression). Coordination keys (generation counter, genat stamp, `aggstale` markers) STAY on the durable shared/coordination Redis — never a lossy/evicting cache (this repo's `agg:members` corruption lesson). `is_configured`-aware (sentinel mode has no host). Best-effort/never-raise preserved.
