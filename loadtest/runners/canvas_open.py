"""Single-scenario Locust runner: the canvas view-open path only.

Used by ``make smoke-canvas-open`` and ``make stress-canvas``. Wraps
``CanvasOpenTasks`` in an HttpUser so Locust can run it directly via ``-f``.
"""
