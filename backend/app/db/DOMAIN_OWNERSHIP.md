...
| **visualization** | `context_models`, `views`, `view_favourites`, `view_versions` | References ontology + workspace by ID only. `ontology_digest` captures schema fingerprint at save time. `view_versions` holds the history of a view's design (FK to `views`, intra-domain). |
| **aggregation** | `aggregation_jobs`, `data_source_polling_configs`, `reconcile_runs`, `refresh_events` | Job lifecycle. Hot writes (checkpoints). `reconcile_runs` is one row per reconciliation sweep (not per data source), trimmed to 30 days. `refresh_events` is the per-operation audit trail (immutable, best-effort emission); the counts-history read correlates against it by id, never by JOIN. |
| **stats** | `data_source_stats`, `data_source_count_snapshots`, `data_source_count_alerts` | `data_source_stats` is a read-mostly cache, tolerant of staleness. `data_source_count_snapshots` is its append-only twin — one row per observed change (plus an hourly heartbeat), purged by age and per-source cap. It carries denormalised `workspace_id`/`provider_id` so the per-provider rollup never JOINs out of this domain. `data_source_count_alerts` freezes an anomaly's verdict and evidence so it outlives the snapshot that produced it. |
