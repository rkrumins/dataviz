...
export function ViewFileDropOverlay({ show }: { show: boolean }) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40 flex items-center justify-center bg-indigo-500/[0.06] backdrop-blur-[1px]">
      <div className="flex flex-col items-center gap-3 rounded-3xl border-2 border-dashed border-indigo-400 bg-canvas-elevated px-12 py-10 shadow-2xl">
        ...
        <p className="text-xs text-ink-muted">
          A <span className="font-mono">.view.json</span> or <span className="font-mono">.view-package.zip</span> file opens the Import journey
        </p>
      </div>
    </div>
  )
