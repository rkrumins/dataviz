...
- Counts-only fingerprint: count-neutral changes need `--force` (documented in the scripts README).
...
- Never-aggregated sources get cache convergence but no first rebuild from the signal — run the first aggregation via the UI/API.
