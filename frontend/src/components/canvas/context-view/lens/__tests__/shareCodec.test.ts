...
describe('lens shareCodec', () => {
  ...
  it('rejects malformed input instead of throwing (a link can never break the app)', () => {
    ...
    expect(decodeLensShare(encodeLensShare({ entries: ['a'], cursor: 0, mode: 'graph', closed: [], frontier: [], containers: [], frameAll: [], contains: [], framePages: [], frameQueries: [] }).slice(4))).toBeNull()
    ...
    expect(decodeLensShare(forge({ v: 2, entries: ['a'], cursor: 0, mode: 'graph', closed: [], frontier: [] }))).toBeNull()
    expect(decodeLensShare(forge({ v: 1, entries: [], cursor: 0, mode: 'graph', closed: [], frontier: [] }))).toBeNull()
    expect(decodeLensShare(forge({ v: 1, entries: ['a'], cursor: 5, mode: 'graph', closed: [], frontier: [] }))).toBeNull()
    expect(decodeLensShare(forge({ v: 1, entries: ['a'], cursor: 0, mode: 'columns', closed: [], frontier: [] }))).toBeNull()
    expect(decodeLensShare(forge({ v: 1, entries: ['a'], cursor: 0, mode: 'graph', closed: 'x', frontier: [] }))).toBeNull()
  })
  ...
  it('rejects a malformed page/query pair rather than restoring nonsense', () => {
    ...
    const base = { v: 1, entries: ['a'], cursor: 0, mode: 'graph', closed: [], frontier: [] }
  ...
  it('opens a link written before containers or frame modes existed', () => {
    ...
    const legacy = btoa(String.fromCharCode(...new TextEncoder().encode(
      JSON.stringify({ v: 1, entries: ['a', 'b'], cursor: 1, mode: 'graph', closed: [], frontier: [] }),
