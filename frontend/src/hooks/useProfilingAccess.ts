/**
 * Who may read profiling.
 *
 * Mirrors the backend gate exactly. `/api/v1/profiling` carries the
 * Ingestion-surface gate (`_require_ingestion_read`) — the same one Freshness
 * and Job History use — because a workspace data source manager who can
 * already see a source's freshness and its current counts must be able to see
 * how those counts MOVED. Gating this at `system:admin` is what hid the whole
 * feature from the very people who onboard the data.
 *
 * Kept in one place, and asserted against the backend's permission list in
 * `useProfilingAccess.test.ts`, because a front end that gates more tightly than
...
export function useCanReadProfiling(): boolean {
...
export function useCanEditProfilingPolicy(): boolean {
