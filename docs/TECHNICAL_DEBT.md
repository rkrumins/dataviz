...
| **Partial metrics export / no alerting** | **MEDIUM** | `GET /internal/metrics` now serves Prometheus exposition format (`app/observability/`), gated by `INTERNAL_METRICS_ENABLED` and aggregated across gunicorn workers via `PROMETHEUS_MULTIPROC_DIR`. It covers request rate by route template, DB statements by pool role, pool saturation, and held change-feed connections. **Still missing**: any scraper pointed at it, dashboards, and alerts — plus the metrics it does not yet emit: web event-loop lag (the wedge watchdog is still log-only), per-provider reachability, stream/consumer-group lag, Redis memory/eviction, worker fleet size. `jobs/metrics.py` counters are not exported yet either. Resilience you can't observe fails silently, and a metric nobody scrapes is not observed. |
