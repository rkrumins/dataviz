...
class AssignmentEngine:
    ...
    def _resolve_assignment(
        ...
        # 2.5 Node's own persisted layer assignment (EXPLICIT, per-entity).
        #
        # `layerAssignment` is stamped on the node itself and kept current by
        # every explicit user action: create (creation layer) AND move (a layer
        # move now persists as an `update` node op that rewrites this field — see
        # stagedChangesToOps). It is the single source of truth for placement that
        # survives a reload. It MUST outrank the GENERIC rule matching below: a
        # rule keyed on entity TYPE / tag / pattern is a broad default, so letting
        # it override an explicit per-entity assignment made a saved move
        # "disappear" on reload (the entity snapped back to its type-rule layer).
        # Session instance assignments (step 1) and containment inheritance
        # (step 2) still outrank it. Because a move rewrites the field, it is never
        # stale relative to the user's last explicit choice.
