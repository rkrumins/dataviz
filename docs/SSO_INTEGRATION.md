...
| Forged assertion posted by a browser (browser-exchange mode) | mandatory signature verification against the connection's JWKS (`validate_settings` refuses the mode without one), `exp` required, asymmetric algorithms only, and single-use enforcement that fails closed — a copied assertion is spent by the sign-in it was copied from, and prod refuses to serve the mode without a shared replay store | `providers/backchannel.py:identity_from_assertion`, `app/main.py:backchannel_builder_with_replay_cache` |
