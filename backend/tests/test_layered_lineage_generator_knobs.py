"""Unit tests for the additive structural knobs on the layered-lineage generator:
...
def test_default_output_byte_identical():
    model = LayeredLineageModelGenerator(seed=7).generate()
...
def test_default_has_no_orphans():
    model = LayeredLineageModelGenerator(num_layers=3, seed=1).generate()
...
def test_depth_2_is_flat_layer_to_attribute():
    model = LayeredLineageModelGenerator(num_layers=3, seed=1, depth=2).generate()
...
def test_depth_3_is_layer_object_attribute():
    model = LayeredLineageModelGenerator(num_layers=3, seed=1, depth=3).generate()
    prefixes = _prefixes(model)
    assert prefixes.get("OBJ", 0) > 0
    assert prefixes.get("GRP", 0) == 0
    for lid in model["layers"]:
...
def test_depth_5_inserts_two_group_levels():
    model = LayeredLineageModelGenerator(num_layers=2, seed=1, depth=5).generate()
    lid = model["layers"][0]
...
def test_orphans_are_uncontained_nonroot_attributes():
    model = LayeredLineageModelGenerator(num_layers=3, seed=1, orphans=4).generate()
...
def test_depth_and_orphans_deterministic_under_seed():
    a = LayeredLineageModelGenerator(num_layers=3, seed=9, depth=5, orphans=3).generate()
    b = LayeredLineageModelGenerator(num_layers=3, seed=9, depth=5, orphans=3).generate()
