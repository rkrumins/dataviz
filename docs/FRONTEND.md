...
| `LineageLens` | `components/canvas/context-view/LineageLens.tsx` | The focus room: a node's complete lineage laid out sources → focus → consumers, fetched live from `/trace/closure` and walked hands-free. Header controls (Next · Walk · Steps · Density · direction) each carry a `ControlTip` popover. The List body is **deprecated** (2026-08-22): no toggle offers it, `lensViewMode` migrates to `'graph'` (preferences v5), the branch stays one release for rollback |
...
| `lens/focus-layout.ts` | | The board: spine walk, **fan-in bundles** (cards sharing a parent fold under it at Overview/Grouped — `BUNDLE_WINDOW` rows), **partner grain** (`OPEN_PARTNERS_PER_BAND`), band pitch `CARD_W + BAND_GAP` |
| `lens/FocusGraphView.tsx` | | React Flow rendering, `onlyRenderVisibleElements`, the control stack (Center on the focus · zoom · Fit · exports), `readerMoved` tracking |
...
| `lens/LensSkeleton.tsx`, `lens/ControlTip.tsx` | | The picture's ghost under the first fetch; the header's explain-on-hover popovers |
