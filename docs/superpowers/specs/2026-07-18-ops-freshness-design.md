...
The scheduler reconciler additionally skips a marked source when its latest aggregation job failed within the cadence window (`status == "failed" AND now - <latest job's updated_at> < resolve_rebuild_interval(...)`). Migration-free — the state row has NO failure timestamp, so the failure time comes from the latest AggregationJobORM row (worker writes updated_at on every terminal status), the same latest-job lookback H1's failure fields use. Effect: a permanently-failing rebuild is retried at most once per cadence window (e.g. 1h) instead of every tick; the marker is KEPT so the source still reads "needs attention." Escalating backoff is a noted future enhancement.
...
## 10. Cache correctness: physical-graph namespacing + REDIS_CACHE_ routing (user, 2026-07-20)

Concern: the same graph_name on two different physical FalkorDB instances must never share cache. Finding: already prevented today — provider content caches key by `host:port:graph_name` (falkordb_provider `_cache_ns`, with a docstring covering exactly this), and the response cache keys by the globally-unique `data_source_id`. Residual gap: the response cache is NOT robust to a data source being re-pointed to a different physical graph (same ds_id, new host:port:graph). Two tasks fix it strategically:

### 10a. Namespace hardening (H4) — response cache scoped to the physical graph
- `CacheScope` gains `graph_ns: str = ""` — a bounded id of the physical graph = `sha1(f"{host}:{port}:{graph_name}")[:16]` (empty when unknown). `_cache_scope(engine)` populates it from the engine's live provider (`_host`/`_port`/`_graph_name`, same inputs as the provider's `_cache_ns`). `_build_key` / `_build_lkg_key` include `graph_ns` as a key segment. The generation key + marker + genat stay `(ws, ds)`-scoped (coordination; invalidation is by prefix/gen so it covers every graph_ns for a ds). Effect: two physical graphs can never share a response/LKG entry, and a re-point yields a fresh graph_ns → automatic miss/recompute. Optional/defaulted so the service-layer invalidation paths (which construct CacheScope from (ws,ds) without an engine) are unaffected — they invalidate by gen bump + prefix SCAN, both graph_ns-agnostic.

### 10b. REDIS_CACHE_ routing (H5) — payloads on the dedicated cache role
- Split graph_cache's Redis usage: response-cache payloads + LKG use a CACHE-role client resolved via `resolve_redis_config(RedisRole.CACHE)` (honors the global `REDIS_CACHE_*` / legacy `CACHE_REDIS_URL` via `resolve_redis_config(RedisRole.CACHE)`; the per-provider `cacheConnection` override for the RESPONSE cache is a documented FOLLOW-UP — it needs a ds→provider lookup graph_cache doesn't have — so H5 wires the fleet-wide default only), FALLING BACK to the current shared/bus Redis when no cache is configured (no dev regression). Coordination keys (generation counter, genat stamp, `aggstale` markers) STAY on the durable shared/coordination Redis — never a lossy/evicting cache (this repo's `agg:members` corruption lesson). `is_configured`-aware (sentinel mode has no host). Best-effort/never-raise preserved.
