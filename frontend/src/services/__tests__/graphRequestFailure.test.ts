...
describe('isIdempotentGraphRead', () => {
  ...
  it('every GET, and the POSTs that only query', () => {
    ...
    for (const path of ['/nodes/query', '/edges/between', '/nodes/degree', '/nodes/ancestor-chains', '/search/advanced', '/search/membership', '/search/counts', '/trace/v2', '/trace/closure', '/edges/aggregated', '/assignments/compute']) {
