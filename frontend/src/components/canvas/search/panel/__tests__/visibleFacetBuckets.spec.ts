 ...
 *
 * The container roll-up (`by: 'ancestor'`) exists to make the canvas
 * badges and the panel's container counts exact — up to 20 000 buckets
 * of it. Painting that as a card list would bury the results under a
 * wall of folders that the layer › container grouping below already
 * shows, and shows better. So on a canvas that has a session, that
 * facet is badge-only; an ordinary `parent` facet a user builds in
 * Refine still gets its card.
 *
...
describe('visibleFacetBuckets', () => {
    it('drops the ancestor roll-up when a session owns the grouping', () => {
    ...
    it('keeps a `parent` facet as an ordinary facet', () => {
        const buckets = visibleFacetBuckets(
