...
describe('useLayerAssignment — canonical logicalNodeId wraps entities into logical groups', () => {
  ...
  it('leaves an entity visible when its entry names a group the layer does not declare', () => {
    // A group removed from the view must not swallow the entities that
    // still point at it — there is no wrapper to render them inside.
    const res = resolve({
      nodes: [node('e1', 'object')],
      sortedLayers: [layerWithGroups()],
