...
export function RelationshipsPanel({
  ...
  }, [relTypes, search, showStagedOnly, changedIds])
    ...
    <div>
      ...
      {(search || showStagedOnly) && filtered.length > 0 && (
        <p className="text-[11px] text-ink-muted mb-3">
          {filtered.length} of {relTypes.length} relationship{relTypes.length !== 1 ? 's' : ''}{search ? ` match "${search}"` : ''}{showStagedOnly ? ' (staged only)' : ''}
        </p>
      )}
