 ...
 * stack empties, truncation flagging at the per-direction cap,
 * typed-fetch containment/partner-parent queries, and the
...
describe('useLensLineage', () => {
  ...
  it('does not cache URNs getNodes failed to return (they stay retryable)', async () => {
    ...
    // Walking to a second node that reaches 'z' again — because 'z' was
    // never cached, it is requested once more (not stranded).
    getNodes.mockClear()
