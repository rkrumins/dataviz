...
export function TemplateGrid({
    title, subtitle, items, icon: Icon, onBrowseAll, onOpenTemplate,
}: {
    title: string; subtitle?: string
    items: (TemplateBrief | OntologyBrief)[]
    icon: React.ComponentType<{ className?: string }>
    onBrowseAll?: () => void
    /** Called when the featured template tile is clicked. Defaults to deep-linking
     *  into `/templates?focus={id}` so the gallery's detail drawer opens. */
    ...
    const handleOpenTemplate = (id: string) => {
        if (onOpenTemplate) {
            ...
            navigate(`/templates?focus=${encodeURIComponent(id)}`)
        }
    }
                    ...
                    <div className="mb-4">
                        {!('version' in displayItems[0]) ? (
                            <FeaturedTemplateCard
                                template={displayItems[0] as TemplateBrief}
                                onOpen={(t) => handleOpenTemplate(t.id)}
                            />
                        ) : (
