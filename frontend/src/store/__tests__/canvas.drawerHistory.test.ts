...
describe('drawer history', () => {
  it('records each entity the drawer opens on', () => {
    s().openNodeDrawer('a')
    s().openNodeDrawer('b')
    expect(s().drawerHistory).toEqual({ entries: ['a', 'b'], cursor: 1 })
  ...
  it('does not record re-opening the entity already shown', () => {
    s().openNodeDrawer('a')
    s().openNodeDrawer('a')
    expect(s().drawerHistory.entries).toEqual(['a'])
  })
  ...
  it('a new move from the middle drops what was ahead', () => {
    ...
    s().openNodeDrawer('d')
    expect(s().drawerHistory).toEqual({ entries: ['a', 'b', 'd'], cursor: 2 })
  })
  ...
  it('a single-select click joins the same trail', () => {
    ...
    s().selectNode('a')
    s().selectNode('b')
    expect(s().drawerHistory.entries).toEqual(['a', 'b'])
  })
  ...
  it('re-selecting where you just stepped back to does not append a step', () => {
    ...
    expect(s().drawerHistory).toEqual({ entries: ['a', 'b'], cursor: 0 })
    s().drawerForward()
    expect(s().drawerNodeId).toBe('b')
  })
