...
| `lens/LensSkeleton.tsx`, `lens/ControlTip.tsx`, `lens/useToolbarOverflow.ts` | | The picture's ghost under the first fetch; the header's explain-on-hover popovers; the one-row toolbar whose least-used groups fold into the Display menu (`fitToolbar`, measured widths, priority order `TOOLBAR_ORDER`) |
