                        ...
                        {Object.entries(access.effectiveWs).map(([wsId, perms]) => {
                            ...
                            return (
                                <ScopeCard
                                    ...
                                    /* Per-workspace cards link straight to the
                                       workspace detail page so the user can
                                       open it without round-tripping through
                                       /workspaces. */
...
function ScopeCard({
    ...
    monospaceLabel = true, monospaceSublabel = false, href,
}: {
    ...
    /** When set, the header becomes a Link to this destination. */
    href?: string
}) {
    const headerInner = (
        <>
            ...
            {href && <ExternalLink className="w-3 h-3 text-ink-muted shrink-0" />}
        </>
    )
    ...
    return (
        <div className={cn('rounded-xl border bg-glass-base/30 overflow-hidden', `border-${accent}-500/20`)}>
            {href ? (
                <Link
                    to={href}
                    className="flex items-center gap-2.5 px-3 py-2 border-b border-glass-border hover:bg-black/[0.04] dark:hover:bg-white/[0.04] transition-colors group"
                    title={`Open ${scopeLabel}`}
                >
                    {headerInner}
                </Link>
            ) : (
                    ...
                    <div key={b.bindingId} className="flex items-center gap-3 px-3 py-2.5">
                        ...
                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
                            {b.scope.type === 'global' ? (
                                ...
                                <Link
                                    to={`/workspaces/${b.scope.id}`}
                                    className="flex items-center gap-1.5 min-w-0 flex-1 group hover:opacity-90 transition-opacity"
                                    title={`Open ${b.scope.label ?? b.scope.id}`}
                                >
                                    ...
                                    <ExternalLink className="w-2.5 h-2.5 text-ink-muted opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                                </Link>
                            ) : (
                                <>
                                    <Briefcase className="w-3 h-3 text-indigo-500 shrink-0" />
                                    <span className="text-xs font-semibold text-ink truncate">{b.scope.label ?? '—'}</span>
                                </>
                            )}
                        </div>
