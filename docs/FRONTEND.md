...
| `lens/LensSkeleton.tsx`, `lens/ControlTip.tsx`, `lens/ViewControl.tsx`, `lens/useToolbarOverflow.ts` | | The picture's ghost under the first fetch; explain-on-hover popovers; the header's category chips (a chip names the category and its current value and opens a `role=menu` of `menuitemradio` options with meanings — Radix Popover above the Lens); the chips row's fold into a More menu on narrow windows (`fitToolbar`, measured widths, priority `TOOLBAR_ORDER`). Tests choose options through `src/test/lensView.ts` (`chooseView`, `viewValue`) |
