...
- **Eventual consistency with a reconciler.** The scheduler tick (60s) acts on fingerprint drift for scheduled sources and sweeps the stale markers (≤50/tick) for *all* sources: deferred rebuilds fire when the cooldown elapses, failed rebuilds are retried, false markers are cleared, in-flight rebuilds (`pending` state) are left alone. Strictly schedule/event-driven — **read paths never generate unbounded work** (the `110cd431` constraint stands). The insights envelope carries two deliberately bounded exceptions, each capped at one job per scope by a `SET NX` claim: a first-view self-heal on a true cache miss, and a repair enqueue for a row nothing has *attempted* in `DISCOVERY_READ_HEAL_FACTOR` of its own deadlines (i.e. the sweep is demonstrably not reaching it). The defect `110cd431` removed was unbounded — one discovery job per visible row on every 5s poll; neither of these can exceed what the sweep would have done anyway.
