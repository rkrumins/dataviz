...
async def after_projection(data_source_id: str) -> None:
    ...
    generation, which rollup-endpoint keys embed too ("content.rollup"), so one bump covers
    the rollup cells a lineage change moves — then nudge the insights refresh, which also
    marks the materialised top-level payload dirty.
